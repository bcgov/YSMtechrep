...
pspl2 <- pspl1 %>%
  left_join(site[,.(SITE_IDENTIFIER, TSA, TSA_DESC, BEC_ZONE, BEC_SBZ, PROJ_AGE_ADJ)], 
            by = ("SITE_IDENTIFIER"))
...
tass_nores <- tass_output %>%
  filter(SPECIES == "ALL", RESID %in% c("N", NA)) %>%
  ...
  arrange(SITE_IDENTIFIER, VISIT_NUMBER, Year, desc(xy), desc(rust), desc(TASS_ver)) %>%
  group_by(SITE_IDENTIFIER, VISIT_NUMBER, Year) %>%
  slice(1)

tass_nores_1 <- tass_nores %>%
  group_by(SITE_IDENTIFIER) %>%
  filter(VISIT_NUMBER == min(VISIT_NUMBER)) %>%
  ungroup()

tass_nores_2 <- tass_nores_1 %>% 
  expand(nesting(SITE_IDENTIFIER, CLSTR_ID), 
         Year = full_seq(Year, 1))

tass_nores_3 <- tass_nores_2 %>%
  left_join(tass_nores_1, by = c("SITE_IDENTIFIER", "CLSTR_ID", "Year")) 

tass_nores_3_1 <- tass_nores_3 %>%
  left_join(msyt %>% select(CLSTR_ID, ref_age_adj), by = c("CLSTR_ID")) %>%
  mutate(GMV_adj = ifelse((Year-ref_age_adj) < (80-ref_age_adj),
                          GMV*(1-OAF1*((Year - ref_age_adj)/(80 - ref_age_adj)))*(1-OAF2*(Year/100)),
                          GMV*(1-OAF1)*(1-OAF2*(Year/100))))

tass_nores_4_1 <- tass_nores_3_1 %>% 
  filter(!is.na(ref_age_adj)) %>% 
  group_by(SITE_IDENTIFIER) %>% 
  mutate(GMV_proj = zoo::na.approx(GMV_adj, Year, rule = 2)) %>% 
...
currentvol1 <- currentvol %>%
  left_join(tass_nores_4_1 %>% select(-CLSTR_ID, -VISIT_NUMBER, -ref_age_adj),
            by = c("SITE_IDENTIFIER", "PROJ_AGE_ADJ" = "Year")) 
...
currentvol4 <- currentvol3 %>%
  ...
  mutate(
    ...
    tassnv = ifelse(is.na(GMV_proj), 0, GMV_proj),
...
ysm_msyt_vdyp <- currentvol4 %>%
  select(SITE_IDENTIFIER, CLSTR_ID, VISIT_NUMBER, FEATURE_ID_2022,
         ...
         MEAS_YR, PROJ_AGE_ADJ, ref_age_adj, ref_age_cd, PRJ_TOTAL_AGE = PROJ_AGE_ADJ, TASS_age = Age,
         vol_wsv_ha, vol_mer_ha, vol_ntwb_ha,
         vol_wsv_ha_nores, vol_mer_ha_nores,
         vol_ntwb_ha_nores, vdyp_vol_dwb, netvol, volTSR, GMV,GMV_proj,
...
### Leave only one rust and xy per sample
tass_output2 <- tass_output2_1 %>%
  arrange(SITE_IDENTIFIER, VISIT_NUMBER, rust, Age, desc(TASS_ver), desc(xy)) %>%
...
tass_output4_1 <- tass_output4 %>%
  ungroup() %>%
  mutate(age_join = PROJ_AGE_ADJ - Year,
         GMV_adj = ifelse((Age-PROJ_AGE_ADJ) < (80-PROJ_AGE_ADJ),
                          GMV*(1-OAF1*((Age-PROJ_AGE_ADJ)/(80 - PROJ_AGE_ADJ)))*(1-OAF2*(Age/100)),
                          GMV*(1-OAF1)*(1-OAF2*(Age/100))),
         # *adjust tass projection age, to be referenced starting from results age, 
         # and not vegcomp age (which is how tass inputs were originally given);
...
age1 <- tass_output4_1 %>%
  group_by(xy, rust, SITE_IDENTIFIER) %>%
  filter(VISIT_NUMBER == min(VISIT_NUMBER)) %>%
  arrange(xy, rust, SITE_IDENTIFIER, age_adj) %>%
  slice(1) %>%
  ungroup() %>%
  mutate(age_dif = age_adj - ref_age_adj) %>%
  select(xy, rust, SITE_IDENTIFIER, age_dif)

tass_output4_2 <- tass_output4_1 %>%
  left_join(age1, by = c('xy', 'rust', 'SITE_IDENTIFIER')) %>%
  mutate(new_age = age_adj - age_dif)

# *get starting and ending age for linear expansion;
tass_output4_3 <- tass_output4_2 %>%
  arrange(xy, rust, SITE_IDENTIFIER, VISIT_NUMBER) %>% # Ensuring correct order
  group_by(xy, rust, SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID) %>%
  mutate(
  ...
  ungroup() %>%
  select(xy, rust, SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID,
         PROJ_AGE_ADJ, ref_age_adj, age_dif, min_age, max_age)

tass_output4_3_2 <- tass_output4_3 %>%
  arrange(SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID, desc(xy), desc(rust)) %>%
  group_by(SITE_IDENTIFIER, VISIT_NUMBER, rust) %>%
  slice(1)

tass_output4_4 <- tass_output4_3_2 %>%
  rowwise() %>%
  do(data.frame(xy = .$xy, 
                rust = .$rust, 
                ...
                age = seq(.$min_age, .$max_age, by = 1)))

# *merge back with source;
tass_output4_5 <- tass_output4_4 %>%
  left_join(tass_output4_2 %>% select(-PROJ_AGE_ADJ, -ref_age_adj), 
            by = c("xy", "rust", "SITE_IDENTIFIER", "VISIT_NUMBER", "CLSTR_ID", "age" = "new_age")) %>%
  mutate(GMV_adj = ifelse(age == 0 & is.na(GMV_adj), 0, GMV_adj))

# *create annual records from starting age to max age;
tass_output4_6 <- tass_output4_5 %>% 
  group_by(xy, rust, SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID) %>% 
  mutate(GMV_approx = zoo::na.approx(GMV_adj, age, rule = 2),
         Age_adj_approx = zoo::na.approx(age_adj, age, rule = 2),
         Year_approx = zoo::na.approx(Year, age, rule = 2),
         Age_approx = zoo::na.approx(Age, age, rule = 2), 
         age_join_apporx = zoo::na.approx(age_join, age, rule = 2)) %>% 
  fill(xy, rust, TASS_VER, TASS_ver, SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID, .direction = 'up') %>%
  ungroup()

tass_output5 <- tass_output4_2 %>%
  group_by(CLSTR_ID, xy, rust) %>%
  slice(which.min(abs(age_join)))

tass_output6 <- tass_output4_6 %>%
  filter(Year_approx %in% c(30, 40, 50, 60, 70, 80, 90, 100) | 
           Age_adj_approx %in% c(30, 40, 50, 60, 70, 80, 90, 100)) %>%
  select(SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID, xy, rust, TASS_ver, PROJ_AGE_ADJ, ref_age_adj, age,
         GMV_approx, Age_adj_approx, Year_approx, Age_approx, age_join_apporx)

### Leave only one rust and xy per sample, with first visit
tass_output7 <- tass_output6 %>% 
  arrange(SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID, desc(xy), desc(rust)) %>%
  group_by(SITE_IDENTIFIER, Age_adj_approx) %>%
  slice(1)
...
tsr_volproj5_1 <- tsr_volproj4 %>%
  left_join(tass_output6 %>%  
            ...
            by = c("SITE_IDENTIFIER", "CLSTR_ID", "AGE" = "Age_adj_approx")) %>%
...
tsr_tass_volproj <- tsr_volproj5_1 %>%
  select(FEATURE_ID_2022, SITE_IDENTIFIER, VISIT_NUMBER, CLSTR_ID, MEAS_YR, PROJ_AGE_ADJ, 
         ...
         xy, rust, TASS_ver, age, Year_approx, Age_approx, age_join_apporx, GMV_approx, volTASS) %>%
  distinct()
